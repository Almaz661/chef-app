...
`npm run seed` наполняет БД готовым универсальным справочником на ~138
продуктов в 20 категориях (овощи, фрукты, крупы, бобовые, мясо, рыба,
молочное, специи, масла, орехи, семена, сухофрукты, бакалея, соусы,
сладкое, …). Каждый продукт имеет:

- человекочитаемое имя и slug,
- категорию,
- базовую единицу (`g`/`ml`/`pcs`),
- КБЖУ на 100 единиц базовой единицы,
- диетические теги (`vegan`, `vegetarian`, `gluten_free`, `lactose_free`),
- алиасы для распознавания строк рецептов.

Плюс 5 демо-рецептов: томатный салат, паста карбонара, борщ, овсянка с
бананом, греческий салат — для проверки всего пайплайна (рецепт → меню
→ покупки → готовка → история → диета).

Данные разнесены по `prisma/data/`:

```
units.ts             # 8 единиц + глобальные конверсии (kg↔g, l↔ml)
products-fresh.ts    # овощи / фрукты / крупы / мясо / молочное
products-pantry.ts   # специи / масла / соусы / сладкое / орехи / семена
conversions.ts       # ст.л.→г, ч.л.→г, щепотка→г для конкретных продуктов
recipes.ts           # демо-рецепты
```

Целостность каталога проверяется юнит-тестом `test/catalog-integrity.spec.ts`
(уникальные slug, корректные единицы, непустые алиасы, валидные КБЖУ,
все ссылки рецептов и конверсий резолвятся).



### Recipe groups (Phase 6.6)

Кнопки навигации по типу блюда: **Завтраки, Супы, Основные блюда,
Салаты, Выпечка, Десерты, Напитки.** Реализовано закрытым enum-ом
`RecipeGroup` на `Recipe`.

| Метод | Путь | Назначение |
| --- | --- | --- |
| GET | `/recipes?group=BREAKFAST` | Список рецептов одной группы |
| GET | `/recipes?search=суп` | Подстрочный поиск по `title` + `description` (CI). Комбинируется с `group`. |
| GET | `/recipes/groups` | Счётчики `[{ group, count }]` для всех 7 групп + `UNGROUPED`, в фиксированном порядке. Пустые группы возвращаются с `count: 0` — чтобы фронт мог нарисовать все 7 кнопок. |
| POST/PATCH | `/recipes`, `/recipes/:id` | Опциональное поле `group` в теле. |

Демо-каталог seed содержит 19 рецептов, минимум 2 на каждую группу:
BREAKFAST 3, SOUP 3, MAIN 4, SALAD 3, BAKING 2, DESSERT 2, DRINK 2.
